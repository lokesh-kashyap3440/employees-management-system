...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onNavigateToRegister }) => {
  ...
  return (
    <>
      ...
      <div className="max-w-md mx-auto mt-4 px-6 py-3 bg-white/50 backdrop-blur-sm border border-gray-100 rounded-2xl shadow-sm">
        <div className="flex flex-col gap-1 text-[10px] font-mono text-gray-400 uppercase tracking-widest">
          <p className="flex justify-between border-b border-gray-100/50 pb-1 mb-1">
            <span className="font-bold">API Endpoint</span>
            <span className="text-blue-500 lowercase">{API_URL}</span>
          </p>
          <p className="flex justify-between border-b border-gray-100/50 pb-1 mb-1">
            <span className="font-bold">Client ID</span>
            <span>{CLIENT_ID ? 'Configured ✅' : 'Missing ❌'}</span>
          </p>
          <p className="flex justify-between">
            <span className="font-bold">Platform</span>
            <span>{isNative ? 'Native 📱' : 'Web 🌐'}</span>
          </p>
        </div>
      </div>
    </>
